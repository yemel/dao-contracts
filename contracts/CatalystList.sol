...
contract CatalystList is Ownable {
    /// Events
    event AddCatalyst(bytes32 indexed _id, address indexed _owner, string _domain);
    event RemoveCatalyst(bytes32 indexed _id, address indexed _owner, string _domain);

    event Add(address indexed _caller, int256 x, int256 y);
    event Remove(address indexed _caller, int256 x, int256 y);
    ...
    */
    function addCatalyst(address _owner, string calldata _domain) external onlyOwner {
        ...
        catalystIndexById[id] = catalystIds.length - 1;
    ...
    */
    function removeCatalyst(bytes32 _id) external onlyOwner {
